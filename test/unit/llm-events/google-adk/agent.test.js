...
const GoogleAdkLlmAgent = require('#agentlib/llm-events/google-adk/agent.js')
...
test('constructs default instance', async (t) => {
  const event = new GoogleAdkLlmAgent(t.nr)
...
test('sets description when provided', async (t) => {
  t.nr.description = 'A helpful agent'
  const event = new GoogleAdkLlmAgent(t.nr)
  assert.equal(event.description, 'A helpful agent')
})

test('omits description when not provided', async (t) => {
  const event = new GoogleAdkLlmAgent(t.nr)
