...
class InstrumentationTarget {
   ...
   *
   * @param {string} event One of 'start' (fires once, at the call itself - where segments
   * typically get created), or 'end'/'asyncEnd'/'asyncStart'/'error'.
  ...
  on(event, handler) {
    ...
    if (!ALLOWED_EVENTS.includes(event)) {
      throw new TypeError(`subscription for '${this.moduleName}': unknown event '${event}' - must be one of start/${ALLOWED_EVENTS.join('/')}`)
    }
 ...
 * @param {string[]} params.events Lifecycle events (other than 'start') to subscribe to.
