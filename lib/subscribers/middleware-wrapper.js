...
class MiddlewareWrapper {
  ...
  wrap({ handler, prefix, route, segmentName, nextIdx = -1 }) {
    ...
    function wrappedHandler (...args) {
      ...
      const segResult = self.createAndEnterSegment({ handler, prefix, route, errorWare, segmentName, ctx, transaction })
      if (!segResult) {
        return handler.apply(this, args)
      }
      const { newCtx, segment } = segResult
  ...
  /**
   * Creates, starts, and enters a new segment for a middleware handler.
   * Handles naming, recording, CLM attributes, and tracer context setup.
   * Returns `null` when segment creation fails — the caller should fall back
   * to invoking the original handler directly.
   *
   * @param {object} params function parameters
   * @param {Function} params.handler the middleware function being wrapped
   * @param {string} [params.prefix] segment name prefix, falls back to `this.prefix`
   * @param {string} [params.route] route path used for segment naming
   * @param {boolean} [params.errorWare] whether this is error-handling middleware
   * @param {string} [params.segmentName] explicit segment name, bypasses route-based naming
   * @param {Context} params.ctx current tracer context
   * @param {Transaction} params.transaction active transaction
   * @returns {{ segment: TraceSegment, newCtx: Context } | null} the new segment and entered context, or null if segment creation failed
   */
  createAndEnterSegment({ handler, prefix, route, errorWare, segmentName, ctx, transaction }) {
    const name = this.nameSegment({ handler, prefix, route, errorWare, segmentName })
    const recorder = this.constructRecorder({ handler, transaction, segmentName })
    const parent = ctx?.segment
    const segment = this.agent.tracer.createSegment({ name, parent, recorder, transaction })
    if (!segment) {
      this.logger.trace('Failed to create new segment %s, calling original function', name)
      return null
    }
    segment.start()
    this.logger.trace('Created segment %s, parent %s', segment.name, parent?.name)
    if (this.config.code_level_metrics.enabled === true) {
      handler[clm] = true
      addCLMAttributes(handler, segment)
    }
    const newCtx = ctx.enterSegment({ segment })
    return { newCtx, segment }
  }
