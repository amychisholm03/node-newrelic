...
module.exports = class GoogleAdkAgentRunSubscriber extends AiMonitoringSubscriber {
  ...
  /**
   * Wraps the async generator returned by BaseAgent.runAsync() to intercept
   * yielded events and record LlmTool events for function responses.
   * When the generator completes, records the LlmAgent event.
   *
    ...
    generator.next = async function wrappedNext(...args) {
      try {
        ...
        if (result?.done) {
          ...
          self.#maybeRecordToolEvent({ event: result.value, segment, transaction, aiAgentName })
        }
        return result
      } catch (err) {
  ...
  /**
   * Inspects a yielded ADK event for function responses and records
   * `LlmTool` events for each tool call found.
   *
   * @param {object} params function parameters
   * @param {object} params.event The yielded ADK event
   * @param {object} params.segment The current NR segment
   * @param {object} params.transaction The active NR transaction
   * @param {string} params.aiAgentName The AI agent name
   */
  #maybeRecordToolEvent({ event, segment, transaction, aiAgentName }) {
    ...
    for (const part of parts) {
      ...
      const toolName = fnResponse.name
      if (!toolName) {
        continue
      }
      ...
      const toolEvent = new LlmTool({
        ...
        aiAgentName,
        runId: fnResponse.id,
        output: fnResponse.response ? JSON.stringify(fnResponse.response) : undefined
      })
