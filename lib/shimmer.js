...
const createCustomSubscriber = require('./subscribers/create-custom-subscriber')
...
const shimmer = (module.exports = {
  ...
  removeHooks(agent) {
    ...
    if (this._customModulePatches) {
      for (const customModulePatch of this._customModulePatches) {
        customModulePatch.unpatch()
      }
      this._customModulePatches = []
    }
  },
  ...
  setupCustomSubscriber: function setupCustomSubscriber(agent, moduleName, config, events, handlers) {
    const instrumentations = []

    config.instrumentations.forEach((instrumentation, i) => {
      const { functionQuery } = instrumentation
      const targetName =
        functionQuery.methodName || functionQuery.expressionName || functionQuery.functionName
      // We'll create the `channelName` for the user to avoid possible duplication
      const channelName = `nr_custom_${moduleName}_${targetName}`

      instrumentations.push({ ...instrumentation, channelName })

      const CustomSubscriber = createCustomSubscriber({
        channelName,
        events: events[i],
        handlers: handlers[i]
      })
      const subscriber = new CustomSubscriber({ agent, logger, packageName: moduleName })
      subscriber.enable()
    ...
    modulePatch.patch()
    this._customModulePatches = this._customModulePatches || []
    this._customModulePatches.push(modulePatch)
  },
